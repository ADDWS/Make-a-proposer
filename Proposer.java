...
public class Proposer {
    ...
    Proposer(int port, String votingName){
        this.port = port;
    ...
    public void prepare(String acceptIp, int acceptPort, long idPrefix) throws IOException,InterruptedException{
        ...
        System.out.println("Proposer " + proposerName + ": prepare to proposing to " + acceptPort);
        int tryTime = 0;
        ...
        while (tryTime < RETRY_TIMES){
            ...
            try{
                ...
                System.out.println(proposerName + " preparetation cannot connect to server "+ acceptorName +" retry in 3 second");
                Thread.sleep(3000);
                continue;
            }
            break;
    ...
    private void prepareStage(Socket proposer, String message) throws IOException,InterruptedException{
        ...
        if(comingPid > acceptPid){
            if(candidataValue.length() == 0){
                acceptPid = comingPid;
                System.out.println("Accepter " + port_acceptor.get(port) + " promise the id " + acceptPid);
                bw.write(String.valueOf(acceptPid));
                data_out.writeUTF("promise");
            }else{
                acceptPid = comingPid;
                System.out.println("Accepter " + port_acceptor.get(port) + " promise a new id, but there is a value " + candidataValue);
                bw.write(acceptPid + "," + candidataValue);
    ...
    private synchronized void commitStage(Socket proposer, String message) throws IOException,InterruptedException{
        ...
        System.out.println("Accepter" + port_acceptor.get(port) + "is comparing " + acceptPid + " and " + Long.parseLong(canIdPari[1]));
    ...
    public void tellToLearn(String acceptIp, int acceptPort) throws IOException,InterruptedException{
        ...
        while (tryTime < RETRY_TIMES){
            ...
            try{
                ...
                System.out.println(proposerName + "preparation:: cannot connect to acceptor" + acceptorName + ", retry in 3 second.");
                Thread.sleep(3000);
                continue;
            }
            break;
